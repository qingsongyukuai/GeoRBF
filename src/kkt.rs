...
use crate::numerical::{
    EXECUTED_NUMERICAL_POLICY, NumericalPolicyId, SpectralAnalysisFailure, SpectralRankDecision,
        ...
        .filter(|row| {
            ...
            !(0.5..=2.0).contains(&norm)
                && cumulative_exponents[*row].abs()
                    == EXECUTED_NUMERICAL_POLICY.ruiz_cumulative_exponent_limit
        })
        .collect();
    Ok(ScaledKkt {
        matrix,
...
pub(crate) fn solve_equality_kkt(
    ...
    if !scaling_round_trip_error.is_finite()
        || scaling_round_trip_error > EXECUTED_NUMERICAL_POLICY.recovery_round_trip_limit
    {
        let reason = BackendContractViolationReason::ScalingRoundTripExceeded {
            observed: scaling_round_trip_error,
            limit: EXECUTED_NUMERICAL_POLICY.recovery_round_trip_limit,
        };
...
fn solve_lblt_with_refinement(
    ...
    while refinement_steps < EXECUTED_NUMERICAL_POLICY.kkt_max_refinement_steps {
        let error =
            linear_residual_evidence(matrix, dimension, &solution, rhs).normalized_backward_error;
        if !error.is_finite()
            || error <= EXECUTED_NUMERICAL_POLICY.backend_standard_form_backward_error_limit
...
fn candidate_rejection_reason(
    ...
    if candidate.iter().any(|value| !value.is_finite()) {
        ...
        || residual.normalized_backward_error
            > EXECUTED_NUMERICAL_POLICY.backend_standard_form_backward_error_limit
    {
        Some(KktAttemptFailureReason::BackendContract(
            BackendContractViolationReason::BackwardErrorExceeded {
                observed: residual.normalized_backward_error,
                limit: EXECUTED_NUMERICAL_POLICY.backend_standard_form_backward_error_limit,
            },
        ))
    } else {
        None
    ...
    KktAttemptRecord {
        ...
        settings: match kind {
            KktAttemptKind::BunchKaufmanRefinement => BackendAttemptSettings::Lblt {
                ...
                maximum_refinement_steps: EXECUTED_NUMERICAL_POLICY.kkt_max_refinement_steps,
            },
